...
def main():
    ...
    other_notes             = ''      # Notes to be added on the folder version name
    ...
    use_battery_storage     = False              # Use battery storage in the model
    #-------------------
    save_results            = True
    plot_results            = False
    save_images             = True
    #-------------------
    name_version            = f"centralisedv7-{import_price}_{tariff_name}" \
    ...
    fuel_cell =        [p_fc       <= p_fc_max,
                        ...
                        q_gen_fc   == (1 - eff_fc_h2) * eff_fc_th * h2_fc * HHV_H2,
    ...
    electricity_h2_net_demand   = l_h2_net.value            # vector TODO: do we need this?
    ...
    total_electricity_h2_net_demand     = np.sum(electricity_h2_net_demand)         # scalar
    ...
    if save_results:
        ...
        scalar_results = {
            ...
            'total_electricity_h2_net_demand':  (total_electricity_h2_net_demand, 'kWh'),
        ...
        results = pd.DataFrame({"Time": np.arange(0,T,1),
                                ...
                                "LoadH2NetSystem":              electricity_h2_net_demand,
        ...
        fig, ax = plt.subplots()
        ax.stackplot(time_index, electricity_pv_produced, electricity_fc_produced, labels=["PV", "Fuel Cell"], colors=['gold', 'royalblue'])
        ax.set_xlabel("Date")
        ax.set_ylabel("Power [kW]")
        ax.set_title("Electricity Generation")
        ax.legend()
        ...
        fig, ax = plt.subplots()
        ax.stackplot(time_index, cum_electricity_pv_produced, cum_electricity_fc_produced, labels=["PV", "Fuel Cell"], colors=['gold', 'royalblue'])
        ax.set_xlabel("Date")
        ax.set_ylabel("Power [kWh]")
        ...
        fig, ax = plt.subplots()
        ax.stackplot(time_index, heat_el_used, heat_fc_used, heat_hp_used, labels=["Electrolyser", "Fuel Cell", "Heat Pump"], colors=['orangered', 'royalblue', 'darkkhaki'])
        ax.set_xlabel("Date")
        ax.set_ylabel("Heat [kW]")
        ax.set_title("Heat Supply")
        ax.legend()
        ...
        fig, ax = plt.subplots()
        ax.stackplot(time_index, cum_heat_el, cum_heat_fc, cum_heat_hp, labels=["Electrolyser", "Fuel Cell", "Heat Pump"], colors=['orangered', 'royalblue', 'darkkhaki'])
        ax.set_xlabel("Date")
        ...
        for start_date, end_date in weekly_time_frames:
            ...
            # Plot stacked electricity generation
            fig, ax = plt.subplots()
            ax.stackplot(time_index[start_index:end_index], electricity_pv_produced[start_index:end_index], electricity_fc_produced[start_index:end_index], labels=["PV", "Fuel Cell"], colors=['gold', 'royalblue'])
            ax.set_xlabel("Date")
            ax.set_ylabel("Power [kW]")
            ax.set_title("Electricity Generation")
            ax.legend()
            ...
            fig, ax = plt.subplots()
            ax.stackplot(time_index[start_index:end_index], heat_el_used[start_index:end_index], heat_fc_used[start_index:end_index], heat_hp_used[start_index:end_index], labels=["Electrolyser", "Fuel Cell", "Heat Pump"], colors=['orangered', 'royalblue', 'darkkhaki'])
            ax.set_xlabel("Date")
            ax.set_ylabel("Heat [kW]")
            ax.set_title("Heat Supply")
            ax.legend()
