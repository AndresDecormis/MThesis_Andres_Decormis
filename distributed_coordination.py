...
# distributed_coordination is a script that obtains the solution to minimise total operational costs 
# of a multi-energy community with the Alternating Direction Method of Multipliers (ADMM) 
# To learn about ADMM:
#   Boyd, Stephen & Parikh, Neal & Chu, Eric & Peleato, Borja & Eckstein, Jonathan. (2011). 
#   Distributed Optimization and Statistical Learning via the Alternating Direction Method of Multipliers. 
#   Foundations and Trends in Machine Learning. 3. 1-122. 10.1561/2200000016. 
...
def main():
    ...
    min_cost_centralised    = 1769.38  # Minimum cost for the system from the centralised optimisation
    ...
    other_notes             = '-'               # TODO: Write here the name of this scenario
    ...
    dynamic_parameter       = True              # Use dynamic rho values
    # -----------------
    # Type of algorithm for optimisation
    algorithm_type          = 'ADMM'            # 'ADMM' or 'LR' (Lagrangian Relaxation - ignoring augmented lagrangian)
    #-------------------
    save_results            = True              # Save the results in a csv file
    plot_results            = False             # Plot the results
    save_images             = True              # Save the images of the plots
    #-------------------
    name_version            = "distributed" \
    ...
    # rho_heat_values      = [0.01,0.05,0.1]     # Initial value of the penalty parameter
    # rho_elec_values      = [0.01,0.05,0.1]     # Initial value of the penalty parameter
    rho_heat_values      = [0.1]     # Initial value of the penalty parameter
    rho_elec_values      = [0.1]     # Initial value of the penalty parameter
    ...
    for rho_heat_fixed, rho_elec_fixed in rho_combinations:
        ...
        lambda_heat_weighted_storage= np.zeros(max_iter)
        ...
        lambda_elec_weighted_storage= np.zeros(max_iter)
        primal_gap_elec_storage     = np.zeros((T,max_iter))
        primal_norm_elec_storage    = np.zeros(max_iter)

        # Storing norm2 values over iterations
        norm2_dual_residual_heat_storage   = np.zeros(max_iter)
        norm2_primal_residual_heat_storage = np.zeros(max_iter)
        norm2_dual_residual_elec_storage   = np.zeros(max_iter)
        norm2_primal_residual_elec_storage = np.zeros(max_iter)
        ...
        for iter in range(max_iter):
            ...
            dual_residual_heat_h2 = rho_heat.value * (q_h2_last - q_h2_fixed.value)
            dual_residual_heat_h2_norm2 = np.linalg.norm(dual_residual_heat_h2, ord = 2)**2
            dual_residual_elec_h2 = rho_elec.value * (p_h2_last - p_h2_fixed.value)
            dual_residual_elec_h2_norm2 = np.linalg.norm(dual_residual_elec_h2, ord = 2)
            ...
            dual_residual_heat_hp = rho_heat.value * (q_hp_last - q_hp_fixed.value)
            dual_residual_heat_hp_norm2 = np.linalg.norm(dual_residual_heat_hp, ord = 2)**2
            dual_residual_elec_hp = rho_elec.value * (l_hp_last - l_hp_fixed.value)
            dual_residual_elec_hp_norm2 = np.linalg.norm(dual_residual_elec_hp, ord = 2)**2
            ...
            dual_residual_heat_cons1        = rho_heat.value * (q_cons1_last - q_cons1_fixed.value)
            dual_residual_heat_cons1_norm2  = np.linalg.norm(dual_residual_heat_cons1, ord = 2)**2
            dual_residual_elec_cons1        = rho_elec.value * (l_cons1_last - l_cons1_fixed.value)
            dual_residual_elec_cons1_norm2  = np.linalg.norm(dual_residual_elec_cons1, ord = 2)**2
            ...
            dual_residual_heat_cons2        = rho_heat.value * (q_cons2_last - q_cons2_fixed.value)
            dual_residual_heat_cons2_norm2  = np.linalg.norm(dual_residual_heat_cons2, ord = 2)**2
            dual_residual_elec_cons2        = rho_elec.value * (l_cons2_last - l_cons2_fixed.value)
            dual_residual_elec_cons2_norm2  = np.linalg.norm(dual_residual_elec_cons2, ord = 2)**2
            ...
            dual_residual_heat_cons3        = rho_heat.value * (q_cons3_last - q_cons3_fixed.value)
            dual_residual_heat_cons3_norm2  = np.linalg.norm(dual_residual_heat_cons3, ord = 2)**2
            dual_residual_elec_cons3        = rho_elec.value * (l_cons3_last - l_cons3_fixed.value)
            dual_residual_elec_cons3_norm2  = np.linalg.norm(dual_residual_elec_cons3, ord = 2)**2
            ...
            if use_thermal_storage:
                ...
                dual_residual_heat_ts        = rho_heat.value * (q_ts_net_last - q_ts_net_fixed.value)
                dual_residual_heat_ts_norm2  = np.linalg.norm(dual_residual_heat_ts, ord = 2)**2
            ...
            if use_battery_storage:
                ...
                dual_residual_elec_bat      = rho_elec.value * (p_bat_net_last - p_bat_net_fixed.value)
                dual_residual_elec_bat_norm2 = np.linalg.norm(dual_residual_elec_bat, ord = 2)**2
            ...
            dual_residual_elec_grid = rho_elec.value * (p_grid_last - p_grid_fixed.value)
            dual_residual_elec_grid_norm2 = np.linalg.norm(dual_residual_elec_grid, ord = 2)**2
            ...
            primal_residual_heat    = get_average_heat() * n_heat_agents     # r^H,k+1  Difference between heat consumption and production at every timestep [kWh]
            primal_residual_elec    = get_average_elec() * n_elec_agents     # r^E,k+1Difference between electricity consumption and production at every timestep [kWh]
            norm2_dual_residual_heat      = np.sqrt(dual_residual_heat_h2_norm2 + dual_residual_heat_hp_norm2 + dual_residual_heat_cons1_norm2 + dual_residual_heat_cons2_norm2 + dual_residual_heat_cons3_norm2 + dual_residual_heat_ts_norm2) # Norm of the dual residual for heat [kWh]
            norm2_dual_residual_elec      = np.sqrt(dual_residual_elec_h2_norm2 + dual_residual_elec_hp_norm2 + dual_residual_elec_cons1_norm2 + dual_residual_elec_cons2_norm2 + dual_residual_elec_cons3_norm2 + dual_residual_elec_bat_norm2 + dual_residual_elec_grid_norm2) # Norm of the dual residual for electricity [kWh]
            ...
            # Norms of residuals
            norm1_primal_residual_heat  = np.linalg.norm(primal_residual_heat, ord = 1)
            norm2_primal_residual_heat  = np.linalg.norm(primal_residual_heat, ord = 2)
            norm1_primal_residual_elec  = np.linalg.norm(primal_residual_elec, ord = 1)
            norm2_primal_residual_elec  = np.linalg.norm(primal_residual_elec, ord = 2)
            ...
            lambda_heat_weighted_storage[iter]   = lambda_heat_fixed.value @ (q_cons1.value + q_cons2.value + q_cons3.value) / np.sum(q_cons1.value + q_cons2.value + q_cons3.value)
            primal_gap_heat_storage[:,iter]      = primal_residual_heat
            primal_norm_heat_storage[iter]       = norm1_primal_residual_heat

            lambda_elec_storage[:,iter]          = lambda_elec_fixed.value
            lambda_elec_weighted_storage[iter]   = lambda_elec_fixed.value @ (l_cons1.value + l_cons2.value + l_cons3.value) / np.sum(l_cons1.value + l_cons2.value + l_cons3.value)
            primal_gap_elec_storage[:,iter]      = primal_residual_elec
            primal_norm_elec_storage[iter]       = norm1_primal_residual_elec

            norm2_dual_residual_heat_storage[iter] = norm2_dual_residual_heat
            norm2_primal_residual_heat_storage[iter] = norm2_primal_residual_heat
            norm2_dual_residual_elec_storage[iter] = norm2_dual_residual_elec
            norm2_primal_residual_elec_storage[iter] = norm2_primal_residual_elec
            ...
            change_dual_heat           = lambda_heat_fixed.value - lambda_heat_storage[:,iter]
            ...
            change_dual_elec           = lambda_elec_fixed.value - lambda_elec_storage[:,iter]

            # Norms of the residuals
            # norm_primal_residual_heat   = np.linalg.norm(primal_residual_heat, ord = 1)
            norm_change_dual_heat     = np.linalg.norm(change_dual_heat, ord = 1)

            # norm_primal_residual_elec   = np.linalg.norm(primal_residual_elec, ord = 1)
            norm_change_dual_elec     = np.linalg.norm(change_dual_elec, ord = 1)

            # Update penalty parameter
            if dynamic_parameter:
                if norm2_primal_residual_heat > 100 * norm2_dual_residual_heat:
                    rho_heat.value = 2 * rho_heat.value
                elif norm2_dual_residual_heat > 0.1 * norm2_primal_residual_heat:
                    rho_heat.value = 0.5 * rho_heat.value
                else: 
                    rho_heat.value = rho_heat.value
                if norm2_primal_residual_elec > 100 * norm2_dual_residual_elec:
                    rho_elec.value = 2 * rho_elec.value
                elif norm2_dual_residual_elec > 0.1 * norm2_primal_residual_elec:
                    rho_elec.value = 0.5 * rho_elec.value
                else:
                    rho_elec.value = rho_elec.value
                    

            # Check for convergence
            if norm1_primal_residual_heat < tolerance_primal and norm1_primal_residual_elec < tolerance_primal:
                if norm2_dual_residual_elec < tolerance_dual and norm2_dual_residual_heat < tolerance_dual:
                    break
                else:
                    print("Primal residual converged, but dual residual did not.")
            

            print("Iteration: ",                                        iter," finished.\n")
            print("Current norm 1 of primal residual (heat): ",           norm1_primal_residual_heat," \n")
            print("Absolute change in dual this iteration: ",           norm_change_dual_heat," \n")
            print("Current norm 1 of primal residual (electricity): ",    norm1_primal_residual_elec," \n")
            print("Absolute change in dual this iteration: ",           norm_change_dual_elec," \n")
        ...
        # Distributed results
        absolute_deviation_heat     = norm1_primal_residual_heat # scalar
        absolute_deviation_elec     = norm1_primal_residual_elec # scalar
        ...
        if save_results:
            ...
            results_iterations = pd.DataFrame({"Iteration": range(iter),
                                            "Norm1PrimalResidualHeat": primal_norm_heat_storage[0:iter],
                                            "Norm1PrimalResidualElec": primal_norm_elec_storage[0:iter],
                                            ...
                                            "Norm2DualResidualHeat": norm2_dual_residual_heat_storage[0:iter],
                                            "Norm2PrimalResidualHeat": norm2_primal_residual_heat_storage[0:iter],
                                            "Norm2DualResidualElec": norm2_dual_residual_elec_storage[0:iter],
                                            "Norm2PrimalResidualElec": norm2_primal_residual_elec_storage[0:iter],
                                            "WeightedHeatValue": lambda_heat_weighted_storage[0:iter],
                                            "WeightedElecValue": lambda_elec_weighted_storage[0:iter]
                                            })
            # Save to csv
            ...
            scalar_results = {
                ...
                'remaining_primal_residual_heat':   (absolute_deviation_heat, 'kWh'),
                'remaining_primal_residual_elec':   (absolute_deviation_elec, 'kWh'),
