...
name_version    = 'electricity_prices_filtered'          # Name of the version of the results
...
p_el_spot_plus  = p_el_spot + spot_plus_tariff_1         # Electricity export price [CHF/kWh]
...
p_el_iwb        = af.get_iwb_tariff_data(tariff='power small')       # Electricity import price [CHF/kWh]

# Extract useful statistics from each price
# Mean electricity prices
p_el_spot_mean          = np.mean(p_el_spot)
p_el_spot_plus_mean     = np.mean(p_el_spot_plus)
...
p_el_iwb_mean           = np.mean(p_el_iwb)
# Standard deviation of electricity prices
p_el_spot_std           = np.std(p_el_spot)
p_el_spot_plus_std      = np.std(p_el_spot_plus)
...
p_el_iwb_std            = np.std(p_el_iwb)

# Save the statistics in a csv file
df = pd.DataFrame()
df['Tariff name'] = ['Spot', 'SpotPlus', 'Vario Grid', 'Vario Plus', 'DT Plus', 'BKW Green', 'BKW Blue', 'BKW Grey', 'IWB Power Small']
df['Mean electricity price [CHF/kWh]'] = [p_el_spot_mean, p_el_spot_plus_mean, p_el_vario_grid_mean, p_el_vario_plus_mean, 
                                          p_el_dt_plus_mean, p_el_bkw_green_mean, p_el_bkw_blue_mean, p_el_bkw_grey_mean, p_el_iwb_mean]
df['Standard deviation [CHF/kWh]'] = [p_el_spot_std, p_el_spot_plus_std, p_el_vario_grid_std, p_el_vario_plus_std, 
                                      p_el_dt_plus_std, p_el_bkw_green_std, p_el_bkw_blue_std, p_el_bkw_grey_std, p_el_iwb_std]
df.to_csv(f"{folder_version}/electricity_prices_stats.csv", index=False)
...
ax.plot(time_index, p_el_spot_plus, label='Spot Plus', color='darkgoldenrod', linestyle='dotted')
...
ax.plot(time_index, p_el_iwb, label='IWB Power Small', color='brown')
ax.set_xlabel('Time')
...
n_bins = 45
fig, ax = plt.subplots(figsize=(12, 6))
ax.hist(p_el_spot, bins=n_bins, alpha=0.3, label='Spot', color='black')
ax.hist(p_el_spot_plus, bins=n_bins, alpha=0.5, label='Spot Plus', color='darkgoldenrod')
...
ax.axvline(p_el_iwb[0], color='brown', alpha = 1, label='IWB Power Small', linewidth=3)
ax.axvline(p_el_iwb[10], color='brown', alpha = 1, linewidth=3)
